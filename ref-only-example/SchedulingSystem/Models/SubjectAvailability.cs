...
/// <summary>
/// IMPORTANT: Represents time CONSTRAINTS/PREFERENCES for subjects (from UNTIS GPU016.TXT)
/// Negative values = AVOID scheduling, Positive values = TRY TO schedule
///
/// UNTIS Constraint/Preference Scale:
/// -3: AVOID scheduling (strongest - highest penalty, e.g., no lab in early morning)
/// -2: AVOID scheduling (medium strength)
/// -1: AVOID scheduling (weakest - lowest penalty)
///  0: Neutral (no preference either way - can schedule anytime)
/// +1: TRY TO schedule (weakest - lowest bonus)
/// +2: TRY TO schedule (medium strength, e.g., math in morning)
...
public class SubjectAvailability
{
    ...
    /// <summary>
    /// UNTIS importance/constraint scale: -3 (prefer NOT to teach) to +3 (MUST teach)
    /// NEGATIVE values = unavailability/constraints (avoid teaching subject)
    /// POSITIVE values = preferences/requirements (prefer teaching subject)
    /// </summary>
    [Required]
    [Range(-3, 3)]
